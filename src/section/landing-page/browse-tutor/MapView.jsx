            ...
            <div class="flex flex-col items-center hover:scale-110 transition-transform duration-300">
                <div class="relative">
                    ...
                    <div class="w-12 h-12 rounded-full border-4 border-white shadow-lg overflow-hidden relative z-10 bg-white">
                        <img src="${imageUrl}" alt="Tutor" class="w-full h-full object-cover" />
                    </div>
                    <!-- Pin Indicator -->
                    <div class="w-0 h-0 border-l-[12px] border-l-transparent border-r-[12px] border-r-transparent border-t-[16px] border-t-white mx-auto -mt-2 shadow-sm relative z-0"></div>
                    <!-- Inner accent for pin -->
                    <div class="w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[12px] border-t-[#2E7D32] mx-auto -mt-[14px] relative z-20"></div>
                </div>
            </div>
        `,
        className: "custom-tutor-marker",
        iconSize: [48, 64],
        iconAnchor: [24, 64],
    });
};

const MapView = ({ tutors = [] }) => {
    // London coordinates
    const position = [51.505, -0.09];

    // Simulated coordinates for tutors in London area (Leaflet uses [lat, lng])
    const markers = [
